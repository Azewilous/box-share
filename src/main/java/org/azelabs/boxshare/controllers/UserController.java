...
public class UserController {
    ...
    public ResponseEntity<UserDTO> createUser(@RequestBody UserDTO user) {
        UserDTO response = this.invoker.execute(CommandType.CREATE_USER.getName(), user);
        return ResponseEntity.status(202).body(response);
    }

    @GetMapping("/{id}")
    public ResponseEntity<UserDTO> getUserById(@PathVariable Long id) {
        Optional<UserDTO> user = this.invoker.execute(CommandType.GET_USER.getName(), id);
        return user.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PutMapping
    public ResponseEntity<UserDTO> updateUser(@RequestBody UserDTO user) {
        Optional<UserDTO> updatedUser = this.invoker.execute(CommandType.UPDATE_USER.getName(), user);
